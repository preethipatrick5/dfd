...
import image.functional as F
from image import base_paths
...
class PadSquare(BaseTransform):
    def __init__(
        self, color: Tuple[int, int, int] = constants.DEFAULT_COLOR, p: float = 1.0
    ):
        """
        @param color: color of the padded border in RGB values
...
class RandomEmojiOverlay(BaseTransform):
    def __init__(
        self,
        emoji_directory: str = constants.SMILEY_EMOJI_DIR,
        opacity: float = 1.0,
        emoji_size: float = 0.15,
        x_pos: float = 0.4,
        y_pos: float = 0.4,
