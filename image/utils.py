...
from image.asserts import validate_image_path, validate_output_path
import numpy as np
...
def validate_and_load_image(image: Union[str, Image.Image]) -> Image.Image:
    ...
    if isinstance(image, str):
        local_path = pathmgr.get_local_path(image)
...
def get_template_and_bbox(
    ...
    local_template_path = pathmgr.get_local_path(template_filepath)
    template = Image.open(local_template_path)
    local_bbox_path = pathmgr.get_local_path(template_bboxes_filepath)
