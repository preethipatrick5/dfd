...
import image.utils as imutils
from image import constants, asserts, base_paths
from image.io import pathmgr
from image.metadata import get_metadata, get_func_kwargs
...
def apply_lambda(
    ...
    func_kwargs = get_func_kwargs(metadata, func_kwargs)
...
def blur(
    ...
    func_kwargs = get_func_kwargs(metadata, locals())
...
def brightness(
    ...
    func_kwargs = get_func_kwargs(metadata, locals())
    get_metadata(metadata=metadata, function_name="brightness", **func_kwargs)
...
def color_jitter(
    ...
    func_kwargs = get_func_kwargs(metadata, locals())
    get_metadata(metadata=metadata, function_name="color_jitter", **func_kwargs)
...
def hflip(
    ...
    func_kwargs = get_func_kwargs(metadata, locals())
    get_metadata(metadata=metadata, function_name="hflip", **func_kwargs)
...
def meme_format(
        ...
        output_path: Optional[str] = None,
        text: str = "LOL",
        font_file: str = constants.MEME_DEFAULT_FONT,
        opacity: float = 1.0,
        text_color: Tuple[int, int, int] = constants.DEFAULT_COLOR,
        caption_height: int = 250,
        meme_bg_color: Tuple[int, int, int] = constants.WHITE_RGB_COLOR,
        metadata: Optional[List[Dict[str, Any]]] = None,
        bboxes: Optional[List[Tuple]] = None,
        bbox_format: Optional[str] = None,
) -> Image.Image:
    ...
    asserts.validate_rgb_color(text_color)
    asserts.validate_rgb_color(meme_bg_color)
    ...
    local_font_path = pathmgr.get_local_path(font_file)
...
def overlay_emoji(
        image: Union[str, Image.Image],
        output_path: Optional[str] = None,
        emoji_path: str = constants.EMOJI_PATH,
        opacity: float = 1.0,
    ...
    local_emoji_path = pathmgr.get_local_path(emoji_path)
...
def overlay_onto_screenshot(
        image: Union[str, Image.Image],
        output_path: Optional[str] = None,
        template_filepath: str = base_paths.TEMPLATE_PATH,
        template_bboxes_filepath: str = constants.BBOXES_PATH,
...
def overlay_stripes(
        ...
        line_color: Tuple[int, int, int] = constants.WHITE_RGB_COLOR,
        line_angle: float = 0,
    ...
    assert line_type in constants.SUPPORTED_LINE_TYPES, "Stripe type not supported"
    asserts.validate_rgb_color(line_color)
...
def overlay_text(
        image: Union[str, Image.Image],
        output_path: Optional[str] = None,
        text: List[Union[int, List[int]]] = constants.DEFAULT_TEXT_INDICES,
        font_file: str = constants.FONT_PATH,
        font_size: float = 0.15,
        opacity: float = 1.0,
        color: Tuple[int, int, int] = constants.RED_RGB_COLOR,
        x_pos: float = 0.0,
        y_pos: float = 0.5,
        metadata: Optional[List[Dict[str, Any]]] = None,
        bboxes: Optional[List[Tuple]] = None,
    ...
    asserts.validate_rgb_color(color)
    ...
    local_pkl_path = pathmgr.get_local_path(pkl_file)
...
def pad(
        ...
        w_factor: float = 0.25,
        h_factor: float = 0.25,
        color: Tuple[int, int, int] = constants.DEFAULT_COLOR,
        metadata: Optional[List[Dict[str, Any]]] = None,
        bboxes: Optional[List[Tuple]] = None,
        bbox_format: Optional[str] = None,
) -> Image.Image:
...
def sharpen(
    ...
    func_kwargs = get_func_kwargs(metadata, locals())
    get_metadata(metadata=metadata, function_name="sharpen", **func_kwargs)
...
def vflip(
    ...
    func_kwargs = get_func_kwargs(metadata, locals())
    get_metadata(metadata=metadata, function_name="vflip", **func_kwargs)
